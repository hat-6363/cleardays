export default async function handler(req, res) {
  ...
  try {
    const response = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: 'Cleardays <onboarding@resend.dev>',
        to: [email],
        subject: `Your Cleardays sign-in code: ${code}`,
        html: `
          <div style="font-family:'Outfit',sans-serif;max-width:480px;margin:0 auto;padding:40px 20px;background:#FDFAF5;">
            <div style="font-family:Georgia,serif;font-size:28px;font-weight:700;color:#1A1208;margin-bottom:8px;">☀️ Cleardays</div>
            ...
            <div style="border-top:1px solid #E5D9C8;padding-top:20px;font-size:12px;color:#C9B99A;">Cleardays · The summer planner for busy parents</div>
          </div>`
      })
    });
